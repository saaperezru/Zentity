function [W,H,errs,vout] = nmf_convex_2(V, r, varargin)
...
% initialize H based on what we got passed (H = G')
if isempty(H)
    if isempty(H0)
        % if non-negative datam, use NMF-EUC to do init. as suggested in [2]
        if min(V(:)>=0)
            [W1 H] = nmf_euc(V, r, 'niter', 100);
            H = H + 0.2*ones(size(H));
...
if isempty(W)
    if isempty(W0)
        ...
        W0 = (H'/(H*H'));
        W = zeros(size(W0));
        W(W0>0) = W0(W0>=0)+0.2*ones(size(W0(W0>=0)))*sum(W0(W0>=0))./sum(W0(:)>0);
        W = max(W,myeps);
    else
        W = W0;
    end
...
for t = 1:niter
    ...
    if update_H
        ...
        %H = (H' .* sqrt( ((Yp*W)+(H'*W'*Yn*W)) ./ ((Yn*W)+(H'*W'*Yp*W)) ))'; 
        if norm_h ~= 0
            H = normalize_W(H,norm_h);
        end
    end
