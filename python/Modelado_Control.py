...
class ControlNMF:
    ...
    def imagesInstances(self,controlLatentTopic,tam):
        """Return top <tam> most important documents for the given laten topic."""
    ...
    def names(self,controlLatentTopic,tam, clusteringTextual):
        """Retorn top <tam> most important string array for the given laten topic."""
        importatNames = controlLatentTopic.getModalResume(type = clusteringTextual)
        name = []
        for i in xrange(0,tam):
...
class ControlZentity:
    ...
    def __init__(self,DMMName,RTName,collectionControl,tagsConfig,zentityPaths):
        ...
        RT1.addProperty(ZEntidad.ScalarProperty("Title",ZEntidad.DataTypes.STRING,titleExtractor,True,False))
        descriptionExtractor = Extractors.DescriptionExtractor(xmlInfoPath)
        RT1.addProperty(ZEntidad.ScalarProperty("Description",ZEntidad.DataTypes.STRING,descriptionExtractor,True,False))
        #A main textual category extractor
        mainTextualCategoryExtractor = Extractors.MainCategoryExtractor(self.controlCollection.getControlNMFTextual(),LatentTopicsNameSize,True) 
        ...
        mainVisualCategoryExtractor = Extractors.MainCategoryExtractor(self.controlCollection.getControlNMFVisual(),LatentTopicsNameSize,False) 
        ...
        RT1.addProperty(ZEntidad.ScalarProperty("ImageID",ZEntidad.DataTypes.STRING,imageIdExtractor,False,True))
        ...
        for i in range(len(LTSList)):
            ...
            if selected:
                ...
                LTName = self.controlCollection.getControlNMFTextual().names(LT,LatentTopicsNameSize,True)
                for  j in range(min(LTNameTop,len(LTName))):
                    self.importantTags.add(LTName[j])
                LTName.insert(0,"LTT")
                RT1.addProperty(ZEntidad.ScalarProperty("_".join(LTName),ZEntidad.DataTypes.DOUBLE,LTExtractor))
        ...
        for i in range(len(LTSList)):
            ...
            if selected:
                LTExtractor = Extractors.LatentTopicExtractor(LT)
                LTName = self.controlCollection.getControlNMFVisual().names(LT,LatentTopicsNameSize,False)
                for  j in range(min(LTNameTop,len(LTName))):
                    self.importantTags.add(LTName[j])
                LTName.insert(0,"LTV")
                RT1.addProperty(ZEntidad.ScalarProperty("_".join(LTName),ZEntidad.DataTypes.DOUBLE,LTExtractor))
        ...
        for tag in self.detectMostImportantTextualWords(topWords):
            ...
            RT1.addProperty(ZEntidad.ScalarProperty("Tag_"+tag,ZEntidad.DataTypes.BOOLEAN,tagExtractor))
